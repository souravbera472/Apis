...
public class ApproveBookByAdmin {
    ...
    public ResponseEntity<Document> postApproveData(@PathVariable String approveId, @RequestBody Map<String, Object> data) {
        try {
            List<Map<String, String>> bookInfo = (List<Map<String, String>>) data.get("bookInfo");
            //KLogger.info("Book info: "+bookInfo);
            if (bookInfo.size() == 0)
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new Document("message", "No data available"));
            ...
            if (check) {
                return ResponseEntity.status(HttpStatus.CREATED)
                        .body(new Document("user-message", bookInfo.size() + (bookInfo.size() == 1 ? " book" : " books")
                                + " added successfully in your profile"));
            } else {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new Document("user-message", "book/books can't be added due to internal error"));
            }
        } catch (Exception e) {
            KLogger.error(e);
        }
